...
int main()
{
	...
	*/
	...
	std::shared_ptr<BoxCollider>bc = entity->addComponent<BoxCollider>();
	light->addComponent<Orbiter>();
	bc->setSize(vec3(1.1, 1.1, 1.1));
	bc->setOffset(vec3(3, 3, 3));
	...
	std::shared_ptr<NewSound> so = core->getResources()->load<NewSound>(core->getEnvironment()->getRootDirectory()+"\\Resources\\"+"\\dixie_horn.ogg");
	std::shared_ptr<MeshComponent>me = core->getResources()->load<MeshComponent>(core->getEnvironment()->getRootDirectory()+"\\Resources\\"+"\\curuthers.obj");
	std::string test = core->getEnvironment()->getRootDirectory() + "\\Resources\\"+"\\Whiskers.png";
	std::shared_ptr<Material>m = core->getResources()->loadTexture<Material>(test.c_str());
	
	std::shared_ptr<Shaders>sh = core->getResources()->load<Shaders>(core->getEnvironment()->getRootDirectory()+"\\Resources\\"+"\\Shaders.txt");
	std::shared_ptr<Shaders>sh2= core->getResources()->load<Shaders>(core->getEnvironment()->getRootDirectory()+"\\Resources\\"+"\\NormalShaders.txt");
	// TODO
	tr->setMesh(me);
	tr2->setMesh(me);
	tr->setMaterial(m);
